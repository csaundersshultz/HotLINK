...
def process_image(
    vent: tuple[float, float], elevation: int, out_dir: pathlib.Path, file: pathlib.Path
) -> dict:
    """
    ...
    _save_fig(
        mir, mir_image, f"Middle Infrared\n{image_date.strftime('%Y-%m-%d %H:%M')}"
    )

    _save_fig(
        tir, tir_image, f"Thermal Infrared\n{image_date.strftime('%Y-%m-%d %H:%M')}"
    )

    n_mir = support_functions.normalize_MIR(
        mir
    )  # note, normalize also fills in missing pixels
    ...
    prediction = model.predict(
        predict_data
    )  # shape=[batch_size, 24, 24, 3], for 3 predicted classes:background, hotspot-adjacent, and hotspot
    ...
    hotspot_mask = apply_hysteresis_threshold(prob_active, low=0.4, high=0.5).astype(
        "bool"
    )  # hysteresis thresholding active mask
    ...
    # generate results
    mir_analysis = support_functions.crop_center(
        mir, size=24
    )  # crop to output size, for analysis
    tir_analysis = support_functions.crop_center(
        tir, size=24
    )  # So we can get the background TIR brightness temperature
    if hotspot_mask.any():
    ...
    # NOTE: Wavelength is different for MIR and TIR. Also different for MODIS and VIIRS.
    ...
    hotspot_tir_bt = support_functions.brightness_temperature(tir_hotspot, wl=11.45e-6)
    bg_tir_bt = support_functions.brightness_temperature(tir_background, wl=11.45e-6)
...
def get_results(
    ...
    if isinstance(vent, str):
        ...
        dists = support_functions.haversine_np(
            vent[1], vent[0], volcs["lon"], volcs["lat"]
        )
    ...
    download_meta = preprocess.download_preprocess(
        dates, vent, sensor, folder=data_path, output=output_dir
    )
    ...
    mir_analysis = support_functions.crop_center(
        mir_data, size=24, crop_dimensions=(1, 2)
    )
    tir_analysis = support_functions.crop_center(
        tir_data, size=24, crop_dimensions=(1, 2)
    )
    ...
    for idx in range(img_data.shape[0]):
        ...
        hotspot_mask = apply_hysteresis_threshold(
            slice_prob_active, low=0.4, high=0.5
        ).astype("bool")
        ...
        hotspot_mir_bt = support_functions.brightness_temperature(
            mir_hotspot, wl=3.74e-6
        )
        bg_mir_bt = support_functions.brightness_temperature(mir_background, wl=3.74e-6)

        result["MIR Hotspot Brightness Temperature"].append(
            hotspot_mir_bt.mean().round(4)
        )
        result["MIR Background Brightness Temperature"].append(
            bg_mir_bt.mean().round(4)
        )

        # tir hotspot/background brigbhtness temerature analysis
        tir_hotspot = tir_analysis[idx][hotspot_mask]
        tir_background = tir_analysis[idx][~hotspot_mask]
        hotspot_tir_bt = support_functions.brightness_temperature(
            tir_hotspot, wl=3.74e-6
        )
        bg_tir_bt = support_functions.brightness_temperature(tir_background, wl=3.74e-6)

        result["TIR Hotspot Brightness Temperature"].append(
            hotspot_tir_bt.mean().round(4)
        )
        result["TIR Background Brightness Temperature"].append(
            bg_tir_bt.mean().round(4)
        )
